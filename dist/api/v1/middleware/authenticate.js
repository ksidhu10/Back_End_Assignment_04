"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.authenticate = void 0;
const auth_1 = require("firebase-admin/auth");
const error_1 = require("../error/error");
const errorUtils_1 = require("../Utils/errorUtils");
const auth = (0, auth_1.getAuth)(); // ✅ Initialize Firebase auth
/**
 * Middleware to authenticate a user using Firebase ID token.
 */
const authenticate = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const token = (_a = req.headers.authorization) === null || _a === void 0 ? void 0 : _a.split(" ")[1];
    if (!token) {
        return next(new error_1.AuthenticationError("Unauthorized: No token provided", "TOKEN_NOT_FOUND"));
    }
    try {
        const decodedToken = yield auth.verifyIdToken(token);
        res.locals.uid = decodedToken.uid;
        res.locals.role = decodedToken.role || "user"; // ✅ Default role if not set
        next();
    }
    catch (error) {
        return next(new error_1.AuthenticationError(`Unauthorized: ${(0, errorUtils_1.getErrorMessage)(error)}`, (0, errorUtils_1.getErrorCode)(error)));
    }
});
exports.authenticate = authenticate;
